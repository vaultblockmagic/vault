...
export function VaultAssetModal({ open, onClose, selectedRow, fetchAssets }: any) {
  useEffect(() => {
  ...
  const completeVault = async () => {
    ...
    try {
      ...
      console.log(`${selectedRow.tokenAddress}, ${rawBalanceNumber.toString()}, ${selectedRow?.tokenId ?? ""}, ${selectedRow.isERC20}, ${customPwdChecked}, ${checkedAuthOptions}`)

      await batchVaultAndSetMFA(selectedRow.tokenAddress, rawBalanceNumber.toString(), selectedRow?.tokenId ?? "0", selectedRow.isERC20, customPwdChecked, checkedAuthOptions)
        ...
        authOptions.map((option) => {
          if (option.name === clickedName) {
            if (option.custom && option)
              return { ...option, confirm: true, checked: true, otp: customPwd };
          }
          console.log(option);
          return option;
        })
  ...
  const AllocateSection = (numTokens: string, setNumTokens: any) => {
  ...
  const VaultSection = (handleAuthOptionClick: any, checkCustomPwd: any) => {
    return (
      <>
            ...
            <div className="w-full flex flex-col items-center justify-center rounded-lg p-2 px-8">
              ...
              >
                ...
                <div className="flex w-auto gap-2">
                  ...
                  {authOptions
                    .filter((authOption) => authOption.checked)
                    .map((authOption) => {
                      return (
                        <div
          ...
          >
            <div>
              <div className="relative mx-auto mt-6 max-w-screen-2xl rounded-lg border-2 border-sky-700 bg-black px-4 pb-4 pt-5 text-left shadow-xl sm:my-20 sm:w-full sm:max-w-3xl sm:p-6">
                ...
                {showAllocateSection
                  ? AllocateSection(numTokens, setNumTokens)
                  : showSecureSection
                  ? VaultSection(handleAuthOptionClick, checkCustomPwd)
                  : SummarySection()}
              </div>
            </div>
          </Transition.Child>
